<?php
	...
	if (isset($_POST['submit'])) {
		...
		$query = "INSERT INTO `messages` (`msg_id`, `user_id`, `msg`, `post_time`, `group_id`) VALUES (NULL, '" . $userID . "', '" . $message . "', CURRENT_TIMESTAMP, '4');";

		$conn->query($query);

		header("Location: anime_group.php"); //temporary so that a user's message does not get posted twice when they refresh the page
...
<html>
	...
	<body>
			...
			<ul id="submenu">
				<li>
					...
					<ul>
						...
						<li class="active"><a href="anime_group.php">Anime</a></li>
					</ul>
				</li>
			</ul>
		</div>
		...
		<div class = "feed">
			<?php
				$postFeed = "SELECT fname,lname, msg, post_time, msg_id from users inner join messages on users.id = messages.user_id WHERE group_id = 4 ORDER BY msg_id DESC";
		...
		<div class="posting">
			<form action="anime_group.php" method="POST">
