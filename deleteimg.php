...
<html>
	...
	<body>
    <?php
        $fileDestinationDel = "uploads/profile".$profileUsername."*";
        $fileInfoDel = glob($fileDestinationDel);
		$fileExtDel = explode('.', $fileInfoDel);
		$fileActualExtDel = strtolower(end($fileExtDel));
		$allowedDel = array('jpg', 'jpeg', 'gif', 'png');
        
		if(in_array($fileActualExtDel, $allowedDel)) {
			$fileDel = "uploads/profile".$profileUsername.".".$fileActualExtDel;
        ...
        $sql = "UPDATE users SET img='' WHERE username ='".$_SESSION['username']."'";
